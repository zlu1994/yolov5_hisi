    ...
    HI_S32* ps32TmpBuf,HI_S32 *ps32DstScore, HI_S32 *ps32DstRoi, HI_S32 *ps32ClassRoiNum)
{
    ...
    for(i = 0; i < SAMPLE_SVP_NNIE_YOLOV3_REPORT_BLOB_NUM; i++)
    {
        ...
        //SAMPLE_SVP_TRACE_INFO("sample_svp_nnie_software.c %d\n", __LINE__);
        SAMPLE_SVP_TRACE_INFO("sample_svp_nnie_software.c %d,au32GridNumHeight = %d\n", __LINE__, au32GridNumHeight[i]);
        SAMPLE_SVP_TRACE_INFO("sample_svp_nnie_software.c %d,au32GridNumWidth = %d\n", __LINE__, au32GridNumWidth[i]);
        SAMPLE_SVP_TRACE_INFO("sample_svp_nnie_software.c %d,c = %d\n", __LINE__, SAMPLE_SVP_NNIE_YOLOV3_EACH_BBOX_INFER_RESULT_NUM*u32EachGridBbox);
        //printf("a");
        for (h = 0; h < au32GridNumHeight[i]; h++)
        {
            for (w = 0; w < au32GridNumWidth[i]; w++)
            {
                ...
                {                   
                    // if (i==0)
                    // {
                        SAMPLE_SVP_TRACE_INFO("sample_svp_nnie_software.c %d, value= %f, h=%d, w=%d, c=%d\n", __LINE__, (HI_FLOAT)(ps32InputBlob[c*u32ChnOffset+h*u32HeightOffset+w]) / SAMPLE_SVP_NNIE_QUANT_BASE, h, w, c);
                    // }
                    pf32Permute[u32Offset++] = (HI_FLOAT)(ps32InputBlob[c*u32ChnOffset+h*u32HeightOffset+w]) / SAMPLE_SVP_NNIE_QUANT_BASE;
                }
            }
        }
        SAMPLE_SVP_TRACE_INFO("sample_svp_nnie_software.c %d,u32Offset = %d\n", __LINE__, u32Offset);
        ...
        for(j = 0; j < au32GridNumWidth[i]*au32GridNumHeight[i]; j++)
        {
            ...
            for (k = 0; k < u32EachGridBbox; k++)
            {
                ...
                //SAMPLE_SVP_TRACE_INFO("sample_svp_nnie_software.c %d,f32StartX = %f\n", __LINE__, f32StartX);


                //calculate score
                f32ObjScore = SAMPLE_SVP_NNIE_SIGMOID(pf32Permute[u32Offset + 4]);     //0~1之间的float,而我的只有0.5或1
                (void)SVP_NNIE_SoftMax(&pf32Permute[u32Offset + 5], u32ClassNum);
                f32MaxScore = SVP_NNIE_Yolov2_GetMaxVal(&pf32Permute[u32Offset + 5], u32ClassNum, &u32MaxValueIndex);    //0~1之间的float，而我的只有nan或1

                s32ClassScore = (HI_S32)(f32MaxScore * f32ObjScore*SAMPLE_SVP_NNIE_QUANT_BASE);     //s32ClassScore是0～4096之间，代表得分，但我的只有0和4096，有问题。，u32ConfThresh是2048?????
                //filter low score roi
                //SAMPLE_SVP_TRACE_INFO("sample_svp_nnie_software.c %d, s32ClassScore=%d, u32ConfThresh=%d\n", __LINE__, s32ClassScore, u32ConfThresh);
                //SAMPLE_SVP_TRACE_INFO("sample_svp_nnie_software.c %d, f32ObjScore=%f, f32MaxScore=%f\n", __LINE__, f32ObjScore, f32MaxScore);


                if (s32ClassScore > u32ConfThresh)      //得分大于2048，相当于阈值的0.5，因为满的得分的4096.
                {
                    //SAMPLE_SVP_TRACE_INFO("sample_svp_nnie_software.c %d\n", __LINE__);
                    ...
                    //SAMPLE_SVP_TRACE_INFO("sample_svp_nnie_software.c %d,Xmin = %f;Ymin=%f;Xmax=%f;Ymax=%f;ClsScore=%d\n", __LINE__, pstBbox[u32BboxNum].f32Xmin, pstBbox[u32BboxNum].f32Ymin, pstBbox[u32BboxNum].f32Xmax, pstBbox[u32BboxNum].f32Ymax, pstBbox[u32BboxNum].s32ClsScore);
                    u32BboxNum++;
    ...
    SAMPLE_SVP_TRACE_INFO("sample_svp_nnie_software.c %d, u32ClassNum=%d\n", __LINE__, u32ClassNum);
    for (i = 1; i < u32ClassNum+1; i++)   //1类的时候不会进入循环，可能有问题
    // for (i = 0; i < u32ClassNum; i++)
    {
        u32ClassRoiNum = 0;
        for(j = 0; j < u32BboxNum; j++)
        {
            //SAMPLE_SVP_TRACE_INFO("sample_svp_nnie_software.c %d, pstBbox[j].u32Mask=%d, i=%d, pstBbox[j].u32ClassIdx=%d\n", __LINE__, pstBbox[j].u32Mask, i, pstBbox[j].u32ClassIdx);
            if ((0 == pstBbox[j].u32Mask) && (i == pstBbox[j].u32ClassIdx) && (u32ClassRoiNum < u32MaxRoiNum))
            {
                *(ps32DstRoi++) = SAMPLE_SVP_NNIE_MAX((HI_S32)(pstBbox[j].f32Xmin*u32SrcWidth), 0);
                SAMPLE_SVP_TRACE_INFO("sample_svp_nnie_software.c %d, *(ps32DstRoi++)=%d\n", __LINE__, *(ps32DstRoi));
                ...
                *(ps32DstRoi++) = SAMPLE_SVP_NNIE_MIN((HI_S32)(pstBbox[j].f32Ymax*u32SrcHeight), u32SrcHeight);
                SAMPLE_SVP_TRACE_INFO("sample_svp_nnie_software.c %d, *(ps32DstRoi++)=%d\n", __LINE__, *(ps32DstRoi));
                *(ps32DstScore++) = pstBbox[j].s32ClsScore;     //是int型的，0~4096，后边除以4096，得到真正的分值。
                u32ClassRoiNum++;
                //SAMPLE_SVP_TRACE_INFO("sample_svp_nnie_software.c %d, Xmin=%d,pstBbox[j].Ymin=%d,Xmax=%d,Ymax=%d, u32SrcWidth=%d\n", __LINE__, (HI_S32)pstBbox[j].f32Xmin*u32SrcWidth, (HI_S32)pstBbox[j].f32Ymin*u32SrcHeight, (HI_S32)pstBbox[j].f32Xmax*u32SrcWidth, (HI_S32)pstBbox[j].f32Ymax*u32SrcHeight, u32SrcWidth);
            }
        }
        //SAMPLE_SVP_TRACE_INFO("sample_svp_nnie_software.c %d, u32ClassRoiNum=%d\n", __LINE__, u32ClassRoiNum);
    ...
    SAMPLE_SVP_NNIE_YOLOV3_SOFTWARE_PARAM_S* pstSoftwareParam)
{
    ...
    int map_size[3] = {52, 26, 13};   //52,26,13
    for (int yolo_layer_index = 0; yolo_layer_index < 3; yolo_layer_index ++) { // 3 yolo layer 
		...
		for (int anchor_index = 0; anchor_index < anchor_num; anchor_index ++){ // 每个 grid 上有三个 anchor
				...
				for (int coord_x_y = 0; coord_x_y < feature_length; coord_x_y ++){ // feature size 拉直后的长度，如 1600 400 100.
					...
					printf("sample_svp_nnie_software,%d, confidence=%f", __LINE__, confidence);
					if (confidence > 0.3){
                    }
                }
        }
